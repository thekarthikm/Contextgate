...
ContextGate is a security demonstration, so a good deal of its discussion involves people
trying to break each other's reasoning. That is welcome and encouraged — **critique the
architecture as hard as you like.** Rigorous technical disagreement is the point.

Two things to keep separate:

* **Attacking an argument is welcome. Attacking a person is not.** "This ordering has a race
  condition and here is the case" is excellent. "Only an amateur would write this" is not.
* **Report breaks responsibly.** If you find a genuine way through the authorization boundary,
  disclose it privately first as described in [SECURITY.md](SECURITY.md). Publishing a working
  bypass for points, before a fix exists, is a Code of Conduct matter as well as a security one.

Beginners asking basic questions about RAG, authorization or the codebase are welcome here.
Answer them the way you would want to have been answered.
...
project maintainers by opening a
[private report](https://github.com/thekarthikm/Contextgate/security/advisories/new) or by
contacting the repository owner, [@thekarthikm](https://github.com/thekarthikm), directly
